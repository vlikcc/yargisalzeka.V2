...
public class GeminiController : ControllerBase
{
    ...
    // Aşama bazlı ilerleme için Server-Sent Events (SSE) endpointi
    [HttpPost("composite-search-stream")]
    [Produces("text/event-stream")]
    public async Task CompositeSearchStream([FromBody] CompositeSearchRequest request)
    {
        var response = Response;
    response.Headers["Content-Type"] = "text/event-stream";
    response.Headers["Cache-Control"] = "no-cache";
    response.Headers["X-Accel-Buffering"] = "no"; // Nginx buffering kapatma

        var abort = HttpContext.RequestAborted;
        async Task WriteEventAsync(string phase, string? message = null, object? payload = null)
        {
            if (abort.IsCancellationRequested) return;
            var obj = new { phase, message, payload };
            var json = System.Text.Json.JsonSerializer.Serialize(obj, new System.Text.Json.JsonSerializerOptions { PropertyNamingPolicy = System.Text.Json.JsonNamingPolicy.CamelCase });
        ...
        try
        {
            ...
            {
                await WriteEventAsync("error", "Olay metni gerekli");
                return;
            }
            var userId = User.FindFirstValue(ClaimTypes.NameIdentifier) ?? User.FindFirstValue(JwtRegisteredClaimNames.Sub);
            if (string.IsNullOrEmpty(userId))
            {
                await WriteEventAsync("error", "Unauthorized");
                return;
            }

            await WriteEventAsync("started", "İşlem başladı");

            var token = Request.Headers["Authorization"].ToString();
            var sub = _factory.CreateClient("Subscription");
            if (!string.IsNullOrEmpty(token)) sub.DefaultRequestHeaders.Add("Authorization", token);
            ValidateAccessDto? usage = null;
            try { usage = await sub.GetFromJsonAsync<ValidateAccessDto>("api/subscription/usage", abort); } catch { }
            if (usage == null)
            {
                await WriteEventAsync("error", "Subscription service unreachable");
                return;
            }
            if (usage.CaseAnalysisRemaining == 0 || usage.KeywordExtractionRemaining == 0 || usage.SearchRemaining == 0)
            {
                await WriteEventAsync("forbidden", "Limit tükendi (analysis/keywords/search)");
                return;
            }

            // ANALYSIS & KEYWORDS
            await WriteEventAsync("analysis.started", "Olay analizi başlatıldı");
            await WriteEventAsync("keywords.started", "Anahtar kelime çıkarımı başlatıldı");
            ...
            await WriteEventAsync("analysis.completed", "Olay analizi tamamlandı", new { analysis });
            await WriteEventAsync("keywords.completed", "Anahtar kelimeler hazır", new { keywords });

            // SEARCH
            await WriteEventAsync("search.started", "Arama başlatıldı", new { keywords });
            var searchBase = _configuration["SearchService:BaseUrl"] ?? "http://localhost:5043";
            var searchClient = _httpClientFactory.CreateClient();
            if (!string.IsNullOrEmpty(token)) searchClient.DefaultRequestHeaders.Add("Authorization", token);
            List<SearchServiceDecisionDto>? decisionsRaw = null;
            try
            {
                var searchResp = await searchClient.PostAsJsonAsync($"{searchBase}/api/search", new { keywords }, abort);
                if (searchResp.IsSuccessStatusCode)
                {
                    using var doc = System.Text.Json.JsonDocument.Parse(await searchResp.Content.ReadAsStringAsync(abort));
                    if (doc.RootElement.TryGetProperty("decisions", out var decEl))
                    {
                        decisionsRaw = decEl.EnumerateArray().Select(d => new SearchServiceDecisionDto
                        {
            ...
            {
                _logger.LogWarning(exSearch, "CompositeSearchStream Search hata");
            }
            decisionsRaw ??= new List<SearchServiceDecisionDto>();
            await WriteEventAsync("search.completed", "Arama tamamlandı", new { count = decisionsRaw.Count });

            // SCORING
            await WriteEventAsync("scoring.started", "Relevance skorlaması başladı", new { total = decisionsRaw.Count });
            var maxScore = int.TryParse(Environment.GetEnvironmentVariable("COMPOSITE_SCORE_MAX"), out var tmpMax) ? Math.Clamp(tmpMax, 1, 30) : 8;
            var toScore = decisionsRaw.Take(maxScore).ToList();
            var scored = new List<ScoredDecisionResult>();
            var semaphore = new SemaphoreSlim(3);
            int completed = 0;
            var tasks = toScore.Select(async d =>
            {
                await semaphore.WaitAsync(abort);
                try
                {
                    var rel = await _service.AnalyzeDecisionRelevanceAsync(request.CaseText, d.KararMetni);
                    if (rel != null)
                    {
                        var item = new ScoredDecisionResult
                        {
                            Id = d.Id,
                            Title = $"Yargıtay {d.YargitayDairesi} - {d.EsasNo}/{d.KararNo}",
                            Excerpt = d.KararMetni.Length > 300 ? d.KararMetni.Substring(0, 300) + "..." : d.KararMetni,
                        ...
                        lock (scored) { scored.Add(item); }
                    }
                }
                catch (Exception exRel)
                {
                    _logger.LogDebug(exRel, "Relevance scoring hata (stream)");
                }
                finally
                {
                    Interlocked.Increment(ref completed);
                    await WriteEventAsync("scoring.progress", null, new { completed, total = toScore.Count });
                    semaphore.Release();
                }
            });
            await Task.WhenAll(tasks);
            var top3 = scored
                ...
                .Take(3)
                .ToList();
            await WriteEventAsync("scoring.completed", "Skorlama tamamlandı", new { scored = scored.Count });

            // Quota consumption
            try
            {
                await sub.PostAsJsonAsync("api/subscription/consume", new { FeatureType = FeatureTypes.CaseAnalysis }, abort);
                await sub.PostAsJsonAsync("api/subscription/consume", new { FeatureType = FeatureTypes.KeywordExtraction }, abort);
                await sub.PostAsJsonAsync("api/subscription/consume", new { FeatureType = FeatureTypes.Search }, abort);
            }
            catch (Exception exConsume)
            {
                _logger.LogWarning(exConsume, "CompositeSearchStream quota consumption hata");
            }

            await WriteEventAsync("completed", "Tamamlandı", new CompositeSearchResponse { Analysis = analysis, Keywords = keywords, Decisions = top3 });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "CompositeSearchStream genel hata");
            await WriteEventAsync("error", "Beklenmeyen hata");
        }
    }
