...
curl -sS -X POST "${BASE}/api/gemini/analyze-case" "${common_headers[@]}" -d "{\"caseText\": \"${OLAY_TEXT//"/\"}\"}" | jq . || echo FAIL

echo "[2] extract-keywords" >&2
curl -sS -X POST "${BASE}/api/gemini/extract-keywords" "${common_headers[@]}" -d "{\"caseText\": \"${OLAY_TEXT//"/\"}\"}" | jq . || echo FAIL

echo "[3] full search" >&2
curl -sS -X POST "${BASE}/api/search" "${common_headers[@]}" -d "{\"CaseText\": \"${OLAY_TEXT//"/\"}\"}" | jq '{analysis, keywords, totalResults, decisionsCount:(.decisions|length)}' || echo FAIL
